        ...
        parallel(
          ...
          "i18n": {
            node(label: 'docker-1.13') {
              script {
                try {
                  sh '''docker run -i --net=host --name=$BUILD_TAG-i18n -e EXCLUDE="design_elements.pt car_index.pt" -e GIT_SRC="https://github.com/eea/$GIT_NAME.git --branch=$BRANCH_NAME" eeacms/i18ndude'''
                } finally {
